...
(define (initialize-3d)
  ...
  (3d-quadric-normals quadratic 'smooth)
  (3d-quadric-texture quadratic #t)
  (load-texture))
...
(define (draw w p)
  ...
  (case object
    ((cube) (draw-cube))
    ((cylinder) 
     (3d-translate 0.0 0.0 -2.5)
     (3d-cylinder quadratic 1.0 0.0
		  3.0 32 32))
    ((disk)
     (3d-disk quadratic 0.5 1.5 32 32))
    ((sphere)
     (3d-sphere quadratic 1.3 32 32))
    (else
     (set! part1 (+ part1 p1))
     (set! part2 (+ part2 p2))
     (if (> part1 359)
	 ...
	 (begin
	   (set! p1 1)
	   (set! p2 0)))
     (3d-partial-disk quadratic 0.5 1.5 32 32 part1 (- part2 part1)))))
...
(define (events w event p)
  (case event 
    ((key-down)
	 ...
	 ((o)
	  (case object
	    ((cube) (set! object 'cylinder))
	    ((cylinder) (set! object 'disk))
	    ((disk) (set! object 'sphere))
	    ((sphere) (set! object 'partial-disk))
	    (else (set! object 'cube))))
	 (else (set! redraw #f)))
       (if redraw
	   (widget-redraw w))))))
...
(display "Use 'up' and 'down' keys to rotate the cube. Tap 'space' to turn on/off lighting. Press 'b' to switch on/off blending. Use the 'o' key to switch shapes.")
(newline)

(airglow-run)

(3d-delete-quadric quadratic)