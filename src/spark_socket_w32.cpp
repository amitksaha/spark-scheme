...
spark_socket::spark_htons(int argc, Scheme_Object** argv)
{
  ...
  _ret_ = scheme_make_integer(htons((int)hostshort));
...
spark_socket::spark_ntohl(int argc, Scheme_Object** argv)
{
  ...
  _ret_ = scheme_make_integer(ntohl((long)netlong));
...
spark_socket::spark_ntohs(int argc, Scheme_Object** argv)
{
  ...
  _ret_ = scheme_make_integer(ntohs((int)netshort));
...
spark_socket::send(int, Scheme_Object** argv)
{
  ...
  ssize_t sent = ::send(sock, reinterpret_cast<void*>(c_str),
			buffer.length(), flags);
	...
	*/
	...
	if (scheme_list_length(argv[3]) != 2)
	  scheme_wrong_type("setsockopt", "list-of-2-ints",
			    3, argc, argv);
	if (!spark::Utils::int_from_scheme_long((const Scheme_Object*)SCHEME_CAR(argv[3]), 
						(int&)optval.l_onoff))
	  scheme_wrong_type("setsockopt", "list-of-2-ints",
			    3, argc, argv);
	if (!spark::Utils::int_from_scheme_long((const Scheme_Object*)SCHEME_CAR(SCHEME_CDR(argv[3])), 
						(int&)optval.l_linger))
	  scheme_wrong_type("setsockopt", "list-of-2-ints",
			    3, argc, argv);
	ret = ::setsockopt(sock, level, optname, 
